...

{
    ...
    {
       $err[] = "Название статьи должно быть не меньше 3-х символов и не больше 50. Текущая длина ".mb_strlen($_POST['theme']).".";
    }
     if(!isset($_POST['desc']))
 ...
 function replaceBBCode($text_post) {
    ...
    return preg_replace($str_search, $str_replace, $text_post);
     ...
     if(count($err) == 0) {    
        ...
        //в данном случае добавляется долько дата изменения
        $regdate=date('d.m.y в G:i:s');     
 
    $errors=0;
        

$mysqli->query("UPDATE article SET article_desc='".$desc."', article_name='".$theme."', article_content='".$content."',  article_creator='".$creator."', article_ndate='".$regdate."'WHERE article_id='".$_GET['id']."'");
         //Создание страницы
